...
func TestScanDeterministicAndClassifies(t *testing.T) {
	...
	os.WriteFile(filepath.Join(d, "a.json"), []byte(`{"schema":"capsule-v1","id":"a","status":"BAD"}`), 0644)
	x, e := Scan(d)
	if e != nil {
		t.Fatal(e)
	}
	y, _ := Scan(d)
	if x.Records[0].Path != "a.json" || x.Invalid != 1 || x.BenchmarkReady != 1 || x.UnknownFields != 1 {
